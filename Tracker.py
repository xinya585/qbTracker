...
# ⚠️ 注意：由于判断逻辑已改为"只要有一个正常Tracker即为正常种子"
#        此配置项在当前版本中已被忽略，保留仅为兼容性
...
class QBittorrentChecker:
    ...
    def check_tracker_status(self, torrents: List[Dict[str, Any]] = None, 
        ...
        判断逻辑：只要有一个Tracker正常工作（状态码为2），就认为是正常种子
        只有没有任何一个正常工作的Tracker时，才被标记为问题种子
        
        Args:
            torrents: 种子列表，如果为None则获取所有种子
            min_problem_trackers: 保留此参数仅为兼容性，在当前逻辑中已被忽略
        ...
        print(f"📌 判断标准：只要有一个Tracker正常工作，即视为正常种子\n")
        ...
        for i, torrent in enumerate(torrents, 1):
            ...
            # 🔴 关键修改：只要有一个Tracker正常工作，就认为是正常种子
            # 原逻辑：len(problematic_trackers) >= min_problem_trackers
            # 新逻辑：working_trackers == 0 时才是问题种子（没有任何正常工作的Tracker）
            is_problematic = working_trackers == 0
            
            if is_problematic:
                print(f"⚠️ 问题种子 - 没有任何正常工作的Tracker (共 {total_real_trackers} 个Tracker全部异常)")
                ...
                print(f"✅ 正常种子 - 有 {working_trackers} 个正常工作的Tracker (共 {total_real_trackers} 个Tracker)")
    ...
    def print_problematic_torrents(self, problematic_torrents: List[Dict[str, Any]], 
        ...
        print(f"\n{'='*80}")
        print(f"🔴 发现 {len(problematic_torrents)} 个Tracker存在问题的种子（没有任何正常工作的Tracker）")
        print(f"{'='*80}\n")
    ...
    def print_summary(self, problematic_torrents: List[Dict[str, Any]]):
        ...
        if problematic_torrents:
            print(f"\n🔴 发现 {len(problematic_torrents)} 个有问题的种子（没有任何正常工作的Tracker）：")
            for i, torrent in enumerate(problematic_torrents, 1):
...
def main():
    ...
    parser.add_argument('--min-trackers', type=int, default=DEFAULT_MIN_TRACKERS, help='最少问题Tracker数（已弃用，保留仅为兼容）')
